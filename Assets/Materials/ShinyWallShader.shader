	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(vertOutput output) : COLOR
			{
				...
				float4 color =	lerp(float4(1, 0, 1, 1), float4(0.5, 0, 0, 1), tex2D(_ShinyTex, (output.pos * 0.0005f + _WorldSpaceCameraPos.xy * 0.012f + unity_ObjectToWorld._m03_m13_m23 * 0.03) * float2(2, 0.3)));
				color +=		lerp(float4(1, 0, 1, 1), float4(0.5, 0, 0, 1), tex2D(_ShinyTex, (output.pos * 0.001f + _WorldSpaceCameraPos.xy * 0.008f + unity_ObjectToWorld._m03_m13_m23 * 0.03) * float2(2, 0.3)));
				color +=		lerp(float4(1, 0, 1, 1), float4(0.5, 0, 0, 1), tex2D(_ShinyTex, (output.pos * 0.003f + _WorldSpaceCameraPos.xy * 0.005f + unity_ObjectToWorld._m03_m13_m23 * 0.03) * float2(2, 0.2)));
				color *= 0.333;
				color = lerp(color, 1, tex2D(_MainTex, output.uv));
