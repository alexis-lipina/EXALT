...
public class SolProjectileHandler : BulletHandler
{
    ...
    [SerializeField] AudioClip SFXOnIgnite;
    ...
    public AnimationCurve _preIgniteVelocityOverTime;
    ...
    private void PreIgniteState()
    {
        ...
        if (!HasBeenHit)
        {
            _projectilePhysics.Speed = _preIgniteVelocityOverTime.Evaluate(_stateTimer / _timeUntilIgnite) * _speedUntilIgnite;
        }
        ...
        if (_stateTimer > _timeUntilIgnite || _projectilePhysics.HasHitEnemy)
        {
            ...
            _projectilePhysics.GetComponent<AudioSource>().clip = SFXOnIgnite;
            _projectilePhysics.GetComponent<AudioSource>().Play();
    ...
    private void IgniteState()
    {
        ...
        if (_stateTimer > _igniteShockwaveTimer)
        {
            _projectilePhysics.GlowSprite.material.SetFloat("_Opacity", _igniteGlowBrightnessCurve.Evaluate((_igniteShockwaveTimer / _igniteShockwaveInterval) % 0.5f + 0.5f));
            _projectilePhysics.GlowSprite.transform.localScale = Vector3.one * _igniteGlowScaleCurve.Evaluate((_igniteShockwaveTimer / _igniteShockwaveInterval) % 0.5f + 0.5f);
        }
        else
        {
        ...
        bool bDamagingPulse = _igniteShockwaveTimer > _igniteShockwaveInterval;
        if (bDamagingPulse)
        {
    ...
    public override void OnPlayerDeflect()
    {
        _currentSolProjectileState = SolProjectileState.IGNITE;
        _stateTimer = 0.0f;
        _igniteShockwaveTimer = 0.0f;
        base.OnPlayerDeflect();
    }
}
